...
public class SVGUtil {
    ...
    public Path parsePath(ArrayList<String> svgDataList, float widthFactor, float heightFactor) {
        ...
        while (fp != null) {
            ...
            switch (fp.pathType) {
                ...
                case CURVE_TO: {
                    path.cubicTo(fp.p1.x * widthFactor, fp.p1.y * heightFactor, fp.p2.x * widthFactor, fp.p2.y * heightFactor,
                            fp.p3.x * widthFactor, fp.p3.y * heightFactor);
                    lastPoint = fp.p3;
                    break;
                }
                case QUAD_TO: {
                    path.quadTo(fp.p1.x * widthFactor, fp.p1.y * heightFactor, fp.p2.x * widthFactor, fp.p2.y * heightFactor);
                    lastPoint = fp.p2;
                    break;
                }
                case CLOSE: {
    ...
    private FragmentPath nextFrag(ArrayList<String> svgData, int startIndex, Point lastPoint) {
        if (svgData==null)
            return null;
    ...
    public Path parseFragList(ArrayList<FragmentPath> svgStartDataList,ArrayList<FragmentPath> svgEndDataList,float widthFactor,float heightFactor,float animatorFactor){
        Path path=new Path();
        ...
        for (int i=0;i<svgStartDataList.size();i++){
            ...
            int y3=0;
            if (startFp.p1!=null){
                x1= (int) (startFp.p1.x+(endFp.p1.x-startFp.p1.x)*animatorFactor);
                y1= (int) (startFp.p1.y+(endFp.p1.y-startFp.p1.y)*animatorFactor);

            }
            if (startFp.p2!=null){
                x2= (int) (startFp.p2.x+(endFp.p2.x-startFp.p2.x)*animatorFactor);
                y2= (int) (startFp.p2.y+(endFp.p2.y-startFp.p2.y)*animatorFactor);

            }
            if (startFp.p3!=null){
                x3= (int) (startFp.p3.x+(endFp.p3.x-startFp.p3.x)*animatorFactor);
                y3= (int) (startFp.p3.y+(endFp.p3.y-startFp.p3.y)*animatorFactor);
            ...
            switch (startFp.pathType){
                ...
                case CURVE_TO:{
                    path.cubicTo(x1*widthFactor,y1*heightFactor,x2*widthFactor,y2*heightFactor,x3*widthFactor,y3*heightFactor);
                    break;
                }
                case QUAD_TO:{
                    path.quadTo(x1*widthFactor,y1*heightFactor,x2*widthFactor,y2*heightFactor);
                    break;
                }
                case CLOSE:{
                    path.close();
